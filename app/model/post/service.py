...
class PostService(object):
    ...
    def gen_posts(cls):
        ...
        for post_file_name in new_posts:
            if '.html' in post_file_name:
                ...
                post_html_path = os.path.join(config.base_dir, 'data/html/' + post_file_name)
                with open(post_html_path, 'r') as source_file:
                    text = source_file.read()
                soup = BeautifulSoup(text, 'lxml')
                post['year'], post['timestamp'] = time_str2timestamp(soup.find('p').get_text())
                ...
                template = f"""
                    ...
                    {{% end %}}
                    """
                with open(os.path.join(config.base_dir, 'template/post/%s.html' % post['title']), 'w') as template_file:
    ...
    def get_total_page(cls):
        total_page = int((len(cls.gen_posts()) + const_config.PAGE_LIMIT - 1) / const_config.PAGE_LIMIT)
        return total_page
