...
class BasicHandler(tornado.web.RequestHandler):
            ...
            if content_type and content_type.find(r"application/json") >= 0:
                try:
                    ...
                    if isinstance(json_dict, dict):
                        ...
                        raise error_util.ResponseError(error_util.ERROR_CODE.JSON_PARAMS_NOT_IN_DICT)
                except orjson.JSONDecodeError:
                    raise error_util.ResponseError(error_util.ERROR_CODE.JSON_PARAMS_FORMAT_ERROR)
    ...
    def all_arguments(self):
        if self._all_arguments is None:
            self._all_arguments = {key: self.get_argument(key) for key in self.request.arguments.keys()}
    ...
    def validate_argument(self, schema, error=None, data=None, ignore_extra_keys=True):
        ...
        try:
            return Schema(schema, error=error, ignore_extra_keys=ignore_extra_keys).validate(data)
    ...
    def success(self, data, status=const_config.HTTPCode.OK):
        ...
        response = {
            ...
            "message": error_util.ERROR_MAP.get(error_util.ERROR_CODE.SUCCESS)["message"],
            "body": data
        }
        return self.finish(orjson.dumps(response))
